...
function patchFile(relativePath, kind) {
  ...
  if (!source.includes('from "../panel-alert-sounds"')) {
    const importLine = kind === "admin"
      ? 'import { NEW_ORDER_SOUND, ORDER_CANCELLED_SOUND, NEW_ORDER_FALLBACK_SOUND, ORDER_CANCELLED_FALLBACK_SOUND } from "../panel-alert-sounds";\n'
      : 'import { NEW_ORDER_SOUND, ORDER_CANCELLED_SOUND, NEW_ORDER_FALLBACK_SOUND, ORDER_CANCELLED_FALLBACK_SOUND } from "../panel-alert-sounds";\n';
  ...
  if (kind === "admin") {
    ...
    const newBlock = 'const orders=result.orders as Order[];const incoming=orders.filter(order=>!knownOrders.current?.has(order.orderCode)&&order.status==="ACCEPTED");const cancelled=orders.filter(order=>knownStatuses.current?.get(order.orderCode)!=="CANCELLED"&&order.status==="CANCELLED");if(knownOrders.current&&alerts){if(incoming.length){playPanelSound(NEW_ORDER_SOUND,NEW_ORDER_FALLBACK_SOUND);void showBackgroundNotification({title:"SABKA DELIVERY · New order",body:`${incoming[0].storeName} · ${money(incoming[0].total)} · ${incoming[0].orderCode}`,url:"/super-admin",tag:`admin-order-${incoming[0].orderCode}`});}if(cancelled.length){playPanelSound(ORDER_CANCELLED_SOUND,ORDER_CANCELLED_FALLBACK_SOUND);void showBackgroundNotification({title:"SABKA DELIVERY · Order cancelled",body:`${cancelled[0].storeName} · ${cancelled[0].orderCode}`,url:"/super-admin",tag:`admin-cancel-${cancelled[0].orderCode}`});}}knownOrders.current=new Set(orders.map(order=>order.orderCode));knownStatuses.current=new Map(orders.map(order=>[order.orderCode,order.status]));setData(result);';
...
console.log("Panel order and cancellation alerts applied.");
