...
patch("app/super-admin/admin-console.tsx", [
  ['"Coupons"|"Commission Slabs"|"Website Editor"|"Panel Users"', '"Coupons"|"Commission Slabs"|"Website Editor"|"Panel Users"'],
  ['"Coupons" | "Website Editor" | "Panel Users"', '"Coupons" | "Commission Slabs" | "Website Editor" | "Panel Users"'],
  ['item==="Coupons"?"%":item==="Commission Slabs"?"₹":item==="Website Editor"?"✎":', 'item==="Coupons"?"%":item==="Commission Slabs"?"₹":item==="Website Editor"?"✎":'],
  ['item === "Coupons" ? "%" : item === "Website Editor" ? "✎" :', 'item === "Coupons" ? "%" : item === "Commission Slabs" ? "₹" : item === "Website Editor" ? "✎" :'],
  ['{tab==="Commission Slabs"&&<iframe', '{tab==="Commission Slabs"&&<iframe'],
...
patch("app/owner-panel/owner-console.tsx", [
  ['"HQ" | "SECURITY" | "OPERATIONS" | "COMMISSION_SLABS"', '"HQ" | "SECURITY" | "OPERATIONS" | "COMMISSION_SLABS"'],
  ['"HQ"|"SECURITY"|"OPERATIONS"', '"HQ"|"SECURITY"|"OPERATIONS"|"COMMISSION_SLABS"'],
  ['<button className={tab === "COMMISSION_SLABS" ? "active" : ""} onClick={() => setTab("COMMISSION_SLABS")}>₹ Commission Slabs</button>', '<button className={tab === "COMMISSION_SLABS" ? "active" : ""} onClick={() => setTab("COMMISSION_SLABS")}>₹ Commission Slabs</button>'],
  ...
  ['{tab === "COMMISSION_SLABS" && <iframe', '{tab === "COMMISSION_SLABS" && <iframe'],
...
console.log("Commission Slabs sections added safely/idempotently.");
