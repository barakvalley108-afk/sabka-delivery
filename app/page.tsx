...
const defaultRewardOffers: RewardOffer[] = [
  {
    id: 1,
...
export default function Home() {
  ...
  const [brandName, setBrandName] = useState("SABKA DELIVERY");
  const [brandLogo, setBrandLogo] = useState("/images/sabka-delivery-logo.png");
  const [heroTitle, setHeroTitle] = useState("Ghar ka swaad, jaldi delivery");
  const [heroBody, setHeroBody] = useState(
    "Local restaurants, fresh ingredients and the taste you love—delivered fast in Lala Bazar.",
  );
  const [heroImage, setHeroImage] = useState("/images/hero-food-collage.png");
  ...
  const [marketSections, setMarketSections] = useState<MarketSection[]>([
    { key: "FOOD", name: "Food", description: "Restaurants and local favourites", image: "/images/hero-food-collage.png", icon: "🍲", isActive: 1, sortOrder: 1 },
    { key: "GROCERY", name: "Grocery", description: "Daily essentials", image: "/images/grocery-daily-needs.png", icon: "🛍️", isActive: 1, sortOrder: 2 },
    { key: "ELECTRONICS", name: "Electronics", description: "Mobiles, accessories and gadgets", image: "", icon: "⚡", isActive: 1, sortOrder: 3 },
  ]);
  ...
  const loadMarket = useCallback(async () => {
    try {
      ...
      const activeSection=(data.sections||[]).find((section:{key:string;deliveryCharge?:number;minOrder?:number})=>section.key===mode);
      ...
      if (content.branding) {
        ...
        setBrandLogo(content.branding.image || "/images/sabka-delivery-logo.png");
      }
      if (content.homepage_banner) {
        setHeroTitle((current) => content.homepage_banner.title || current);
  ...
  if (selectedCoupon && !couponEligible) discount = 0;
  ...
  const total = subtotal + checkoutDeliveryFee - discount;
...
export async function POST(request: Request) {
  try {
    ...
    const commonQuery = `
      SELECT
        ...
        s.name storeName,
        coalesce(ss.section_key, p.vertical, s.type) storeType,
      ...
      LEFT JOIN market_store_sections ss
        ON ss.store_id = s.id

      LEFT JOIN market_store_profiles p
        ON p.store_id = s.id

      LEFT JOIN market_delivery_assignments a
        ON a.order_code = o.order_code

      LEFT JOIN market_riders r
        ON r.id = a.rider_id
    ...
    const result = isMobileNumber
      ...
      : await db
          .prepare(
            `${commonQuery}
             WHERE UPPER(o.order_code) = UPPER(?)
             ORDER BY o.created_at DESC
             LIMIT 1`,
          )
          .bind(search)
    ...
    return Response.json(
      {
        error: "Order history load nahi hui",
      },
      {
        status: 500,
      },
  ...
  useEffect(() => {
    if (!historyOpen || historyMobile.length !== 10) return;
    let active = true;
    const refresh = async () => {
      try {
        const response = await fetch("/api/market-history", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ mobile: historyMobile }),
          cache: "no-store",
        });
        const data = await response.json();
    ...
    void refresh();
    // Keep active order tracking responsive without reloading the catalog.
          ...
          <aside className="cart-drawer" onClick={(e) => e.stopPropagation()}>
            ...
            {checkout === "success" ? (
              ...
              <form className="checkout-form" onSubmit={placeOrder}>
                ...
                <section className="checkout-coupons">
                  ...
                  <div className="manual-coupon">
                    ...
                    />
                    <button type="button" onClick={applyManualCoupon}>
                      Apply
                    </button>
                  </div>
                  <div>
              ...
              <>
                ...
                <section className="cart-coupons">
      ...
      {historyOpen && (
        <div className="overlay" onClick={() => setHistoryOpen(false)}>
          ...
          >
            ...
            <p>Checkout mein use kiya gaya mobile number daalo.</p>
