  ...
  useEffect(() => {
    const onClick = (event: MouseEvent) => {
      ...
      setData({
        ...
        price,
        category,
        addSelector: "article.product-card",
      });
      setOpen(true);
    };
  ...
  return (
    <div className="product-quick-view-overlay" onClick={() => setOpen(false)}>
      <article className="product-quick-view" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="product-quick-image" style={data.image ? { backgroundImage: `url(${data.image})` } : undefined} />
        <div className="product-quick-content">
          <small>SABKA DELIVERY · PREMIUM PICK</small>
          <h2>{data.name}</h2>
          <div className="product-quick-price">{data.price}</div>
          <p>{text}</p>
          <div className="product-quick-actions">
            <button className="product-quick-add" onClick={buyNow}>BUY NOW</button>
            <button className="product-quick-cart" onClick={() => { setOpen(false); const match = Array.from(document.querySelectorAll("article.product-card")).find((card) => card.querySelector("h3")?.textContent?.trim() === data.name); match?.querySelector<HTMLButtonElement>(".price-row button:not(:disabled)")?.click(); }}>ADD TO CART</button>
          </div>
        </div>
      </article>
    </div>
