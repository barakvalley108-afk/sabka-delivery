...
export default async function ElectronicsPanel() {
  const session = await getPanelSession("RESTAURANT");
  if (!session) redirect("/panel-login");
  if (session.role !== "RESTAURANT" || session.panelType !== "ELECTRONICS")
    redirect(panelRoute(session));
  return <PartnerConsole expected="ELECTRONICS" />;
}
