...
export async function GET() {
  ...
  const [
    ...
    db.prepare(
      `SELECT i.* FROM market_items i
       ...
       LEFT JOIN market_store_profiles sp ON sp.store_id=s.id
       JOIN market_sections section ON section.key=coalesce(ss.section_key,sp.vertical,s.type)
       WHERE i.is_active=1 AND section.is_active=1 ORDER BY section.sort_order,i.store_id,i.id`,
    ),
    db.prepare(
    ...
    db.prepare(
      `SELECT p.code,p.title,p.discount_type discountType,p.discount_value discountValue,p.min_order minOrder,r.max_discount maxDiscount,r.expires_at expiresAt,r.store_id storeId,r.first_order_only firstOrderOnly FROM market_promotions p LEFT JOIN market_promotion_rules r ON r.code=p.code WHERE p.is_active=1 AND (r.expires_at IS NULL OR date(r.expires_at)>=date('now')) AND r.user_mobile IS NULL AND coalesce(r.show_on_website,1)=1 ORDER BY p.created_at DESC`,
    ),
    db.prepare(
  ...
  const config = Object.fromEntries(
    settings.results.map((x) => [String(x.key), String(x.value)]),
  );
