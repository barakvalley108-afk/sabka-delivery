  ...
  try {
    ...
    if (config.maintenance_mode === "true") {
      return Response.json(
        {
          error:
            "Website maintenance chal raha hai. Thodi der baad try karo.",
        },
        { status: 503 },
      );
    }
    ...
    const store = await db
      .prepare(
        `SELECT
           ...
           s.is_open isOpen,
           coalesce(c.approved,0) approved,
           coalesce(c.blocked,0) blocked,
           op.opening_time openingTime,
           op.closing_time closingTime,
           CASE
             WHEN op.store_id IS NULL
               OR op.opening_time IS NULL
               OR op.closing_time IS NULL
             THEN 1
             WHEN op.opening_time<=op.closing_time
               AND time('now','+5 hours','+30 minutes')
                 BETWEEN op.opening_time AND op.closing_time
             THEN 1
             WHEN op.opening_time>op.closing_time
               AND (
                 time('now','+5 hours','+30 minutes')>=op.opening_time
                 OR time('now','+5 hours','+30 minutes')<=op.closing_time
               )
             THEN 1
             ELSE 0
           END withinHours
    ...
    if (!store || !store.approved || store.blocked) {
      return Response.json(
        {
          error:
            "Ye shop abhi order ke liye available nahi hai. Support se contact karo.",
        },
        { status: 409 },
      );
    }
    ...
    if (!store.isOpen) {
      return Response.json(
        {
          error:
            "Shop abhi offline hai. Is samay order accept nahi ho raha.",
        },
        { status: 409 },
      );
    }

    if (!store.withinHours) {
      return Response.json(
        {
          error:
    ...
    if (
      ...
      result.results.some((entry) => entry.store_id !== storeId)
    ) {
      return Response.json(
    ...
    const subtotal = result.results.reduce(
      (sum, entry) =>
        sum +
        (entry.discount_price ?? entry.price) * quantities[entry.id],
      0,
    );
    ...
    const indiaHour =
      (now.getUTCHours() +
        5 +
        (now.getUTCMinutes() >= 30 ? 1 : 0)) %
      24;

    const nightExtra =
      indiaHour >= 22 || indiaHour < 6 ? service.night_charge : 0;

    const rainExtra =
      config.rain_mode === "true" ? service.rain_charge : 0;

    const configuredDeliveryCharge = Number(
      config[`delivery_charge_${store.vertical}`] ??
    ...
    const deliveryFeeBeforeReward =
      (subtotal >= service.free_delivery_above
        ? 0
        : baseDeliveryCharge) +
      nightExtra +
      rainExtra;
    ...
    const sectionMinimum = Math.max(
      0,
      Number(config[`minimum_order_${store.vertical}`] || 0),
    );
    ...
    const deliveryFee = appliedReward
      ? 0
    ...
    if (couponCode) {
      const promo = await db
        .prepare(
          `SELECT
             ...
             p.discount_value discountValue,
             p.min_order minOrder,
             p.is_active isActive,
             r.max_discount maxDiscount,
             r.expires_at expiresAt,
             r.user_mobile userMobile,
             r.store_id storeId,
             r.first_order_only firstOrderOnly,
             r.auto_pause_after_use autoPauseAfterUse
      ...
      if (subtotal < promo.minOrder) {
        return Response.json(
          {
            error:
              `Coupon ke liye minimum order ₹${promo.minOrder} hai`,
          },
          { status: 400 },
        );
      }
      ...
      if (claimed) {
        return Response.json(
          {
            error:
              "Ye coupon is mobile number par pehle use ho chuka hai",
          },
          { status: 409 },
        );
      }
      ...
      autoPauseCoupon = Boolean(promo.autoPauseAfterUse);
    }
    ...
    const orderCode =
      `SD-${Date.now().toString().slice(-7)}-` +
      `${Math.floor(10 + Math.random() * 90)}`;

    const payment =
      body.paymentMethod === "UPI" ? "UPI" : "COD";
    ...
    const statements = [
      ...
      ...(couponCode
        ? [
            ...
            db
              .prepare(
                "UPDATE market_promotions SET uses=uses+1 WHERE code=?",
              )
              .bind(couponCode),
      ...
      ...(appliedReward
        ? [
            db
              .prepare(
                `INSERT INTO market_reward_claims
                 (offer_id,mobile,order_code,cycle_number)
                 VALUES (?,?,?,?)`,
              )
              .bind(
                appliedReward.id,
            ...
            db
              .prepare(
                "UPDATE market_reward_offers SET uses=uses+1 WHERE id=?",
              )
