...
export async function POST(request:Request){
  ...
  await db.batch([
    db.prepare("DELETE FROM market_sessions WHERE user_id=?").bind(user!.id),
    db.prepare("INSERT INTO market_sessions (token_hash,user_id,expires_at) VALUES (?,?,?)").bind(tokenHash,user!.id,new Date(Date.now()+30*86400000).toISOString()),
  ]);
