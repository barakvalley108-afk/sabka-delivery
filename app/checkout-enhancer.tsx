...
export default function CheckoutEnhancer() {
  useEffect(() => {
    ...
    const updateTotals = () => {
      ...
      const payment = form.querySelector<HTMLInputElement>('input[name="payment"]:checked')?.value || "COD";
      if (payment === "UPI" && selectedTip !== 0) {
        selectedTip = 0;
        sessionStorage.setItem(TIP_KEY, "0");
      }

      form.querySelectorAll<HTMLButtonElement>(".rider-tip-option").forEach((button) => {
        button.classList.toggle("active", Number(button.dataset.tip || 0) === selectedTip);
      ...
      const totalBox = form.querySelector<HTMLElement>(".checkout-total");
      const totalValue = totalBox?.querySelector<HTMLElement>("b");
      if (totalValue) {
        const current = Number(totalValue.textContent?.replace(/[^0-9.]/g, "") || 0);
        const savedBase = Number(totalValue.dataset.baseTotal || 0);
        const base = savedBase || Math.max(0, current - selectedTip);
        totalValue.dataset.baseTotal = String(base);
      ...
      if (placeButton && !placeButton.textContent?.includes("Placing")) {
        ...
        const savedBase = Number(placeButton.dataset.baseTotal || 0);
        const base = savedBase || Math.max(0, current - selectedTip);
        placeButton.dataset.baseTotal = String(base);
    ...
    const buildTipBox = () => {
      const form = document.querySelector<HTMLFormElement>(".checkout-form");
      if (!form || form.querySelector(".rider-tip-box")) return;
      ...
      box.querySelectorAll<HTMLButtonElement>(".rider-tip-option").forEach((button) => {
      ...
      custom?.addEventListener("input", () => {
        const value = Math.min(500, Math.max(0, Number(custom.value.replace(/\D/g, "")) || 0));
      ...
      form.querySelectorAll<HTMLInputElement>('input[name="payment"]').forEach((radio) => {
    ...
    window.fetch = async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.includes("/api/market-orders") && init?.method === "POST" && response.ok && selectedTip > 0) {
        try {
          ...
          if (orderCode && requestBody.mobile) {
            await originalFetch("/api/order-tip", {
              ...
              body: JSON.stringify({ orderCode, mobile: requestBody.mobile, tip: selectedTip }),
            });
            selectedTip = 0;
            sessionStorage.removeItem(TIP_KEY);
          }
        } catch {
          // Order must still succeed even if optional tip update fails.
        }
      }
      return response;
    };
